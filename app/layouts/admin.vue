<template>
  <div class="flex min-h-screen bg-base-200">
    <aside class="hidden lg:flex w-64 flex-col border-r border-base-content/10 bg-base-100 px-5 py-6">
      <NuxtLink to="/" class="flex items-center gap-3 font-semibold text-lg">
        <img src="/image.png" alt="DNS" class="h-8 w-8" width="32" height="32">
        <div>
...
v-for="item in navItems" :key="item.to" :to="item.to"
          class="flex items-center gap-3 rounded-xl px-3 py-2 transition" :class="isActive(item.to)
      ...
      <NuxtLink to="/" class="btn btn-outline mt-auto">Вернуться на сайт</NuxtLink>
    </aside>
      ...
      <header
        ...
        <div class="flex items-center gap-3 lg:hidden">
          ...
          <div class="flex gap-1">
            <NuxtLink
v-for="item in navItems" :key="`mobile-${item.to}`" :to="item.to" class="btn btn-xs"
